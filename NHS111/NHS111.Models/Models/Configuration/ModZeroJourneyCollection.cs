...
namespace NHS111.Models.Models.Configuration
{
    public class ModZeroJourneyCollection : ConfigurationElementCollection, IEnumerable<ModZeroJourneyElement>
    {
        ...
        public ModZeroJourneyElement GetModZeroJourneyElement(string age, string gender, bool isTrauma)
        {
            var element = this.FirstOrDefault(j => j.Age == age && j.Gender == gender && j.IsTraumaJourney == isTrauma);
            if(element == null) throw new ConfigurationErrorsException($"A module zero journey has not been defined for {gender} {age} with {(isTrauma ? "Trauma" : "Non-Trauma" )}");
